...
function buildEducation() {
  ...
  const entries = sorted.map(edu => {
    const years = edu.year_end == null
      ? `${edu.year_start}--present`
      : edu.year_end === edu.year_start
        ? String(edu.year_start)
...
function buildWork() {
  ...
  const entries = filtered.map(w => {
    const years = w.year_end == null
      ? `${w.year_start}--present`
...
function buildProjects() {
  ...
  const items = filtered.map(proj => {
    ...
    if (!proj.year_start) {
      yearRange = '';
    } else if (proj.year_end == null) {
      yearRange = `${proj.year_start} to present`;
    } else if (proj.year_end === proj.year_start) {
      yearRange = String(proj.year_start);
    } else {
      yearRange = `${proj.year_start}--${proj.year_end}`;
    }
